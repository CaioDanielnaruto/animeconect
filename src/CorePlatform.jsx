...
export default function Platform() {
  ...
  const saveEvent = async (event) => {
    ...
    const payload = { organizer_id: user.id, venue_id: venueId, title: eventForm.title.trim(), description: eventForm.description.trim(), category: eventForm.category, cover_url: eventForm.cover_url || null, starts_at: new Date(eventForm.starts_at).toISOString(), ends_at: eventForm.ends_at ? new Date(eventForm.ends_at).toISOString() : null, capacity: eventForm.capacity ? Number(eventForm.capacity) : null, status: eventForm.status }
    const result = editingEventId
      ? await supabase.from('events').update(payload).eq('id', editingEventId).eq('organizer_id', user.id)
      : await supabase.from('events').insert({ ...payload, slug: `${slugify(eventForm.title)}-${Date.now().toString(36)}` })
    const { error } = result
    setBusy(false); if (error) return notify('error', friendlyError(error.message))
    setEventForm(blankEvent); setEditingEventId(null); setModal(null); await loadPublic(); notify('success', editingEventId ? 'Evento atualizado!' : payload.status === 'published' ? 'Evento publicado!' : 'Rascunho salvo!')
  }
  ...
  const startEditEvent = (event) => {
    if (!user || event.organizer_id !== user.id) return
    setEditingEventId(event.id)
    setEventForm({
      title: event.title || '', description: event.description || '', category: event.category || 'Festival',
      cover_url: event.cover_url || '', starts_at: toDateTimeInput(event.starts_at), ends_at: toDateTimeInput(event.ends_at),
      capacity: event.capacity || '', status: event.status || 'published', venue_name: event.venue_name || '',
      address_line: event.address_line || '', city: event.city || '', state: event.state || '',
    })
    setModal('create')
  }

  const deleteEvent = async (event) => {
    if (!user || event.organizer_id !== user.id || !window.confirm(`Excluir “${event.title}” permanentemente?`)) return
    setBusy(true)
    const { error } = await supabase.from('events').delete().eq('id', event.id).eq('organizer_id', user.id)
    setBusy(false)
    if (error) return notify('error', friendlyError(error.message))
    setEvents((items) => items.filter((item) => item.id !== event.id))
    setSelectedEvent(null); setModal(null); notify('success', 'Evento excluído permanentemente.')
  }

  return <main>
    <nav className="nav shell"><a className="brand" href="#inicio"><span className="brand-mark">A</span><span>ANIME<span>CONECT</span></span></a><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Abrir menu">☰</button><div className={`nav-links ${menuOpen ? 'open' : ''}`}><a href="#eventos">Eventos</a><a href="#comunidades">Comunidades</a><a href="#sobre">Sobre</a>{authReady && user ? <><button className="login user-chip" onClick={() => setModal('profile')}>Olá, {displayName}</button><button className="primary small" onClick={startCreateEvent}>Criar evento</button><button className="login" onClick={() => supabase.auth.signOut()}>Sair</button></> : <><button className="login" onClick={() => setAuthMode('login')}>Entrar</button><button className="primary small" onClick={() => setAuthMode('signup')}>Criar conta</button></>}</div></nav>
    ...
    <section className="section shell" id="eventos"><div className="section-heading"><div><span className="kicker">AGENDA ANIMECONECT</span><h2>Encontre seu próximo evento</h2></div>{user && <button className="secondary" onClick={startCreateEvent}>+ Criar evento</button>}</div><div className="filters"><input aria-label="Buscar eventos" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar evento, cidade ou estado..."/><select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.map((item) => <option key={item}>{item}</option>)}</select></div>{events[0]?.id?.startsWith('demo-') && <p className="demo-note">Eventos de demonstração — publique o primeiro evento para substituir esta lista.</p>}<div className="event-grid">{filteredEvents.map((item, index) => <EventCard event={{ ...item, cover_url: item.cover_url || themeImages[index % Math.max(themeImages.length, 1)]?.url }} onOpen={() => { setSelectedEvent(item); setModal('event') }} key={item.id}/>)}</div>{!filteredEvents.length && <div className="empty-state">Nenhum evento encontrado.</div>}</section>
